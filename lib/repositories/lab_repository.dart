...
abstract class ILabRepository {
  ...
  Future<ResponseCallback<Lab>> updateLab(Lab lab, int id);
}
...
class LabRepository implements ILabRepository {
  ...
  @override
  Future<ResponseCallback<Lab>> updateLab(Lab lab, int id) async {
    Uri url =
    Uri.http(CommonStrings.apiAuthority, "/api/Labinfo/${id.toString()}");
    ResponseCallback<Lab> responseCallback = ResponseCallback();
    try {
      var r = {
        "lab_name": lab.labName,
      ...
      var req = lab.toJson();
      final response =
          await http.put(url, headers: _headers, body: jsonEncode(r));
      responseCallback.code = response.statusCode;
      var responseMap = jsonDecode(response.body);
      LimsLogger.log("*** Lab updated successfully => $responseMap");
      responseCallback.data = lab;
    } on http.ClientException catch (e) {
      LimsLogger.log(
          "*** http.ClientException in Lab Repository updateLab().");
      LimsLogger.log("Message => ${e.message}");
      LimsLogger.log("Uri => ${e.uri}");
      responseCallback.message = e.message;
      responseCallback.uri = e.uri;
